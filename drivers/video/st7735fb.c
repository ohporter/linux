				...
				int xs, int ys, int xe, int ye)
{
	...
	st7735_write_data(par, xs);
	st7735_write_data(par, 0x00);
	st7735_write_data(par, xe);
	st7735_write_cmd(par, ST7735_RASET);
	st7735_write_data(par, 0x00);
	st7735_write_data(par, ys);
	st7735_write_data(par, 0x00);
	st7735_write_data(par, ye);
}
...
static const struct of_device_id st7735fb_dt_ids[] = {
	{ .compatible = "adafruit,tft-lcd-1.8-green", .data = (void *) ST7735_DISPLAY_AF_TFT18, },
};
...
static int __devinit st7735fb_probe (struct spi_device *spi)
{
	...
	of_id = of_match_device(st7735fb_dt_ids, &spi->dev);
	if (!of_id) {
		pr_err("%s: device is not compatible\n", DRVNAME);
		return -EINVAL;
	}
