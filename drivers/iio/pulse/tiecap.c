...
static const unsigned long ecap_available_scan_masks[] = { 0x00000001, 0x0 };

static ssize_t ecap_attr_show(struct device *dev,
			      struct device_attribute *attr, char *buf)
{
	struct ecap_state *state = iio_priv(dev_to_iio_dev(dev));
...
static irqreturn_t ecap_trigger_handler(int irq, void *private)
{
	...
	*(u32 *)state->buf = readl(state->regs + CAP2);
...
static int ecap_probe(struct platform_device *pdev)
{
	...
	idev->num_channels = 2;
	idev->available_scan_masks = ecap_available_scan_masks;
